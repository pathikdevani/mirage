...
// ---------------- Context ----------------
// `workspaceSchemas` and `customFunctions` are stable for the whole editor — we
// supply them once via context so every nested SegmentEditor (the row's faker
// cell AND every arg field inside an ArgsPopover) can offer cross-schema $refs
// and custom-function chips without prop drilling.
interface SegmentEditorEnv {
  workspaceSchemas: Schema[];
  customFunctions: CustomFunction[];
}
const SegmentEditorContext = createContext<SegmentEditorEnv | null>(null);

export function SegmentEditorProvider({
  workspaceSchemas,
  customFunctions,
  children,
}: SegmentEditorEnv & { children: ReactNode }) {
  const value = useMemo<SegmentEditorEnv>(
    () => ({ workspaceSchemas, customFunctions }),
    [workspaceSchemas, customFunctions],
  );
  return <SegmentEditorContext.Provider value={value}>{children}</SegmentEditorContext.Provider>;
}

function useSegmentEditorEnv(): SegmentEditorEnv {
  return useContext(SegmentEditorContext) ?? { workspaceSchemas: [], customFunctions: [] };
}
...
export interface SegmentEditorProps {
  ...
  /** Local sibling fields available as `{kind:'field'}` chips. */
  siblingFields: RefField[];
  /** The field that owns this expression — excluded from the local-field picker. */
  ownFieldName: string;
  placeholder?: string;
  invalid?: boolean;
  /** Override outer wrapper classes (e.g. for a smaller pill inside args). */
  className?: string;
}
...
export function SegmentEditor({
  ...
  const { workspaceSchemas, customFunctions } = useSegmentEditorEnv();
...
const linkIcon = (): SVGSVGElement =>
  svgIcon('M9 17H7A5 5 0 0 1 7 7h2M15 7h2a5 5 0 1 1 0 10h-2M8 12h8');
const codeIcon = (): SVGSVGElement => svgIcon('M16 18l6-6-6-6M8 6l-6 6 6 6');
...
function SegmentPicker({
    ...
    for (const f of fields) {
      out.push({
        ...
        render: () => (
          <>
            <span
              className={cn('h-1.5 w-1.5 rounded-sm', TYPE_DOT[f.type] ?? 'bg-brand-violet/50')}
            />
  ...
  return createPortal(
    <>
      ...
      <div
        className="fixed z-[71] overflow-hidden rounded-lg border border-border bg-popover shadow-2xl"
